...
To compare these models further, I found the mean error and standard deviation on the error for each frequency from each model. Models 1 through 6 are in order: (K=50, CL=2, Act=Lin), (K=50, CL=4, Act=Lin), (K=10, CL=2, Act=Lin), (K=10, CL=4, Act=Lin), (K=50, CL=2, Act=ReLu), and 3NN. The higher the mean error, indicating the average difference from the true value, the worse the fit. The standard deviation on the error indicates the precision of the model. Smaller deviation means more precise with its predictions. The behavior between the frequencies are consistent, with the largest errors for the radial direction.  
